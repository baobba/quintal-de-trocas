...
class CI_DB_active_record extends CI_DB_driver {
	...
	function _on_duplicate($table, $keys, $values)
    {
        foreach (array_combine($keys, $values) as $key => $value) { $update[] = "{$key} = {$value}"; }
        
        return $this->_insert($table, $keys, $values)." ON DUPLICATE KEY UPDATE ".implode(', ', $update);
    }
	...
	function on_duplicate($table = '', $set = NULL)
    {
        ...
        $sql = $this->_on_duplicate($this->_protect_identifiers($table, TRUE, NULL, FALSE), array_keys($this->ar_set), array_values($this->ar_set));
