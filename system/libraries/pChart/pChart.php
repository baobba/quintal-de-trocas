<?php
 /*
     ...
     Graph setup methods :
      ...
      set_label(&$Data,&$DataDescription,$SerieName,$ValueName,$Caption,$R=210,$G=210,$B=210)
 ...
 */
 ...
 define('FONTCHART', FCPATH . 'system' . DIRECTORY_SEPARATOR . 'pChart' . DIRECTORY_SEPARATOR . 'fonts' . DIRECTORY_SEPARATOR . 'pf_arma_five.ttf');
...
// * Fixed in 1.27 *         { $X2 = $this->GArea_X2 - 1; $Y2 = $this->GArea_Y2 - ($this->GArea_X2 - $X1); }
   ...
   function set_label(&$Data,&$DataDescription,$SerieName,$ValueName,$Caption,$R=210,$G=210,$B=210)
    {
     /* Validate the Data and DataDescription array */
     $this->validateDataDescription("set_label",$DataDescription);
     $this->validateData("set_label",$Data);
   ...
   function drawBasicPieGraph(&$Data,&$DataDescription,$XPos,$YPos,$Radius=100,$DrawLabels=PIE_NOLABEL,$R=255,$G=255,$B=255,$Decimals=0)
    {
     ...
     foreach ($TopPlots as $Key => $Value)
      { foreach ($TopPlots[$Key] as $Key2 => $Value2) { set_type($TopPlots[$Key][$Key2],"float"); } }
   ...
   function Render($FileName)
    {
     ...
     imagepng($this->Picture, FCPATH . 'pchart-images' . DIRECTORY_SEPARATOR . $FileName);
    }
