...
class admin_news extends Model {
	...
	public function form($label = 'Adicionar')
	{
		...
		$formx->add_select('cms_news_author_id')
		      ->set_label('Autor')
		      ->set_value(CmsNewsAuthor::getCombo())
		      ->set_validates(array('required'));
		
		$formx->add_select('cms_news_category_id')
		      ->set_label('Categoria')
		      ->set_value(CmsNewsCategory::getCombo())
		      ->set_validates(array('required'));
		
		$formx->add_file('cover_image')
			  ->set_label('Imagem Capa');
		
		$formx->add_text('publicated_at')
              ->set_label('Date de publica&ccedil;&atilde;o')
        	  ->set_html_class('fulltime')
        	  ->set_validates(array('required'));
		
		$ordering = CmsNews::countAll();
		...
		$formx->add_select('show_footer')
		      ->set_label('Exibe na Rodapé?')
		      ->set_value(ConstHelper::getYesNoHeavyDescription()->getArrayCopy())
		      ->set_validates(array('required'));
	...
	public function create($hasPermission = false)
	{
		...
		if ($formx->is_post()) {
			if (!$formx->has_error()) {
			    ...
			    $cover_image = null;
			    if ($formx->use_field('cover_image')->upload_image(array('uploadPath' => './uploads/image', 'width' => 196, 'height' => 196))) {
			        $cover_image = $formx->use_field('cover_image')->get_posted();
			    }
			    
			    $entity = new $this->entity();
			    $entity->setName($formx->use_field('name')->get_posted());
			    $entity->setCms_news_author_id($formx->use_field('cms_news_author_id')->get_posted());
			    $entity->setCms_news_category_id($formx->use_field('cms_news_category_id')->get_posted());
			    $entity->setFriendly_url(slug($entity->getName()));
			    $entity->setContent($formx->use_field('content')->get_posted());
			    $entity->setCover_image($cover_image);
			    $entity->setShow_footer((int) $formx->use_field('show_footer')->get_posted());
			    ...
			    $entity->setFriendly_url(sprintf('%d-%s', $entity->getId(), $entity->getFriendly_url()));
			    $entity->update();
				
				#$return = $this->setNotification('Registro inserido' . $backBtn, 'success', true);
				redirect(base_url() . 'admin/' . $this->getUrl());
				exit;
		...
		if ($total) {
			...
			if ($entities->count()) {
				...
				foreach ($entities as $entity) {
					...
					$CI->table->add('data de publica&ccedil;&atilde;o', date_create_from_format('Y-m-d H:i:s', $entity->getPublicated_at())->format('d-m-Y'));
					$CI->table->add('orderna&ccedil;&atilde;o', 	 $entity->getOrdering());
					$CI->table->add('ativo', 	     $entity->getActive() ? 'Sim' : 'N&atilde;o');
					$CI->table->add('exibe no rodap&eacute;', $entity->getShow_footer() ? 'Sim' : 'N&atilde;o');
					
					$visualizar = '';
					if ($entity->getCover_image()) {					    
    				    $visualizar = '(<a href="" class="screenshot" rel="' . base_url() . 'uploads/image/' . $entity->getCover_image() . '" onclick="return false;">Imagem T&iacute;tulo</a>)';				    
					}
					$CI->table->add('visualizar', 	$visualizar);
				}
	...
	public function update($id, $hasPermission = false)
	{
		...
		if ($entity->count()) {
			...
			$formx->use_field('cms_news_author_id')->set_selected($entity->getCms_news_author_id());
			$formx->use_field('cms_news_category_id')->set_selected($entity->getCms_news_category_id());
			$formx->use_field('content')->set_value($entity->getContent());
			$formx->use_field('publicated_at')->set_value($entity->getPublicated_at(true, 'd/m/Y H:i:s'));
			$formx->use_field('show_footer')->set_selected($entity->getShow_footer());
			...
			if ($formx->is_post()) {
				if (!$formx->has_error()) {
				    ...
				    $slug = sprintf('%d-%s', $entity->getId(), slug($formx->use_field('name')->get_posted()));
				    
				    $entity->setName($formx->use_field('name')->get_posted());
				    $entity->setCms_news_author_id($formx->use_field('cms_news_author_id')->get_posted());
				    $entity->setCms_news_category_id($formx->use_field('cms_news_category_id')->get_posted());
				    
				    $entity->setFriendly_url($slug);

				    if ($formx->use_field('cover_image')->upload_image(array('uploadPath' => './uploads/image', 'width' => 196, 'height' => 196))) {
				        @unlink(FCPATH . 'uploads' . DIRECTORY_SEPARATOR . 'image' . DIRECTORY_SEPARATOR . $entity->getCover_image());
				        $entity->setCover_image($formx->use_field('cover_image')->get_posted());
				    }
