	...
	public function create($hasPermission = false)
	{
		...
		if ($formx->is_post()) {
			if (!$formx->has_error()) {
			    ...
			    $cover_image = null;
			    if ($formx->use_field('cover_image')->upload_image(array('uploadPath' => './uploads/image', 'width' => 107, 'height' => 107))) {
		...
		if ($total) {
			...
			if ($entities->count()) {
				...
				foreach ($entities as $entity) {
					...
					$CI->table->add('nome',  $entity->getName());
