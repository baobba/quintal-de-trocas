...
class admin_toy_city extends Model {
	...
	public function form($label = 'Adicionar')
	{
		...
		$formx->add_select('cms_toy_state_id')
		      ->set_label('Estado')
		      ->set_value(CmsToyState::getCombo())
		      ->set_validates(array('required'));
	...
	public function create($hasPermission = false)
	{
		...
		if ($formx->is_post()) {
			if (!$formx->has_error()) {
			    ...
			    $entity->setCms_toy_state_id($formx->use_field('cms_toy_state_id')->get_posted());
	...
	public function update($id, $hasPermission = false)
	{
		...
		if ($entity->count()) {
			...
			$formx->use_field('cms_toy_state_id')->set_selected($entity->getCms_toy_state_id());
	...
	public function delete($id = 0, $hasPermission = false)
	{
	    ...
	    $criteria = new Criteria();
		$criteria->setWhere_in('cms_toy_city_id', $ids);
		$criteria->setHydrate(true);
