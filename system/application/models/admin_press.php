...
class admin_press extends Model {
	...
	public function form($label = 'Adicionar')
	{
		...
		$formx->add_select('cms_press_category_id')
		      ->set_label('Categoria')
		      ->set_value(CmsPressCategory::getCombo())
		      ->set_validates(array('required'));
		
		$formx->add_file('cover_image')
			  ->set_label('Imagem Capa');
		
		$ordering = CmsPress::countAll();
	...
	public function create($hasPermission = false)
	{
		...
		if ($formx->is_post()) {
			if (!$formx->has_error()) {
			    ...
			    $entity->setUrl($formx->use_field('url')->get_posted());
	...
	public function update($id, $hasPermission = false)
	{
		...
		if ($entity->count()) {
			...
			$formx->use_field('url')->set_value($entity->getUrl());
			$formx->use_field('cms_press_category_id')->set_selected($entity->getCms_press_category_id());
