...
/* res://connect-min/dev/connect/login.js */

IN.Event.on(IN,$_CONSTANTS.events.systemReady,function(){if(IN.ENV.auth.oauth_token){window.setTimeout(function(){IN.GlobalEvents.auth.fire()
},10)
}});
...
/* res://connect-min/dev/tags/login.js */

Sslac.Class("IN.Tags.Login").Extends("IN.Tags.Base").Constructor(function(g,d){$_STATISTICS.profile("Login");
this.Parent(g,d);
this.label=d.label||"action_linkedin";
this.size=d.size||null;
if(d.content){this.tmpl=this.createTemplate(d.content)
}else{this.tmpl=false
}var h={auth:this.parseAttribute(d.onauth,"callback"),logout:this.parseAttribute(d.onlogout,"callback")};
var c=$_CONSTANTS.events;
for(var e in h){for(var b=h[e],f=0,a=b.length;
f<a;
f++){IN.Event.on(IN,e,b[f])
}}IN.Event.on(IN,c.auth,function(){this.renderLoggedInState()
},this);
IN.Event.on(IN,c.logout,function(){this.renderLoggedOutState()
},this);
if(!IN.User.isAuthorized()){this.renderLoggedOutState()
}$_STATISTICS.profile("Login",true)
}).Method("renderLoggedOutState",function(){this.el().innerHTML="";
var a="",c=$_CONSTANTS.events.click;
switch(this.label.toLowerCase()){case"action":a=$_I18N("sign_in");
break;
case"linkedin":a=$_I18N("linkedin");
break;
case"action_linkedin":default:a=$_I18N("sign_in_with_linkedin");
break
}var b=new IN.Objects.Button({title:a,size:this.size});
b.place(this.el());
IN.Event.on(b,c,function(d){d.preventDefault();
$_STATISTICS.recordAction("Login",c);
IN.User.authorize()
},this)
}).Method("renderLoggedInState",function(a){this.el().innerHTML="";
if(this.tmpl){IN.API.Profile("me").result(function(b){this.el().innerHTML=this.tmpl(b.values[0])
},this).error(function(b){this.el().innerHTML=b.message
},this)
}});
IN.addTag("Login",IN.Tags.Login);

/* res://connect-min/dev/tags/memberdata.js */

Sslac.Class("IN.Tags.MemberData").Extends("IN.Tags.Base").Constructor(function(f,c){$_STATISTICS.profile("MemberData");
this.Parent(f,c);
var b=this;
var e=this.parseAttribute(c.ids);
var a=this.parseAttribute(c.fields)||null;
this.errorCallbacks=this.parseAttribute(c.onerror,"callback");
var g=c.content;
this.tmpl=this.createTemplate(g);
this.makeTwoLegged=true;
this.twoLeggedDone=false;
this.makeThreeLegged=true;
this.threeLeggedDone=false;
if(!IN.User.isAuthorized()){this.makeThreeLegged=false;
this.threeLeggedDone=true;
IN.Event.on(IN,$_CONSTANTS.events.auth,function(){this.makeThreeLegged=true;
this.threeLeggedDone=false;
this.makeCall(false)
},this)
}var d={};
this.mergeResults=function(k,i){for(name in k){if(!d[name]){d[name]={}
}var j=k[name];
var h=d[name];
for(field in j){if(!h[field]||i=="threeLegged"){d[name][field]=j[field]
}}}if(this.twoLeggedDone&&this.threeLeggedDone){this.el().innerHTML=this.tmpl(d)
}};
this.makeCall=function(i){var j=[].slice.apply(e);
var h=IN.API.Profile(j);
if(a){h.fields(a)
}if(i){h.params({isPublic:":public"})
}h.result(function(k){k=k.values;
k=IN.Util.toObject(k,["_key","id","key"],function(m,l){return l.replace($_PATTERNS.prefixes.urlEq,"").replace($_PATTERNS.chars.tilde,"me")
});
if(i){this.twoLeggedDone=true;
this.mergeResults(k,"twoLegged")
}else{this.threeLeggedDone=true;
this.mergeResults(k,"threeLegged")
}},this).error(function(n){if(i){this.twoLeggedDone=true;
this.mergeResults({},"twoLegged")
}else{this.threeLeggedDone=true;
this.mergeResults({},"threeLegged")
}for(var m=0,k=this.errorCallbacks.length;
m<k;
m++){this.errorCallbacks[m](n)
}},this)
};
if(this.makeTwoLegged){this.makeCall(true)
}if(this.makeThreeLegged){this.makeCall(false)
}$_STATISTICS.profile("MemberData",true)
});
...
/* res://connect-min/dev/tags/fullmemberprofile.js */

Sslac.Class("IN.Tags.FullMemberProfile").Extends("IN.Tags.Base").Constructor(function(b,a){$_STATISTICS.profile("FullMemberProfile");
this.Parent(b,a);
this.id=a.id;
this.width=(typeof a.width!==$_CONSTANTS.types.undef)?parseInt(a.width,10):0;
if(isNaN(this.width)){this.width=0
}this.createFrame();
$_STATISTICS.profile("FullMemberProfile",true)
}).Method("createFrame",function(){this.el().innerHTML="";
var a={};
if(this.id){a.member_token=this.id;
if(!IN.ENV.auth.api_key){throw new Error("a valid API key must be provided for FullMemeberProfile")
}a.api_key=IN.ENV.auth.api_key
}else{throw new Error("id must be defined for FullMemeberProfile")
}if(this.width>0){a.width=this.width
}var b=new IN.Objects.SmartWindow({mode:"embedded",url:IN.ENV.widget.full_member_profile_url}).params(a);
b.place(this.el())
});
...
/* res://connect-min/dev/tags/apply.js */

(function(){var j={};
function g(p){var q=[p.companyName||"",p.companyId||"",p.jobTitle||"",p.jobLocation||"",p.jobId||""].join(":");
return""+IN.Util.crc32(q)
}function m(p){return{applied:j[p].applied,sent:j[p].sent}
}function i(q,p,r){if(j[q]){j[q][p]=r
}}function b(p){try{return j[p].tags||[]
}catch(q){return[]
}}function f(q,p){if(!j[q]){j[q]={tags:[],applied:false,sent:false}
}j[q].tags.push(p)
}function k(p,t,s){if(j.hasOwnProperty(p)){var q=b(p),r;
if(t){for(r=q.length;
r--;
){if(!q[r].callbackOnly){q[r].setApplied(true,s)
}}i(p,"applied",s)
}else{for(r=q.length;
r--;
){q[r].setApplied(false)
}j[p]["applied"]=false;
j[p]["sent"]=false
}}}Sslac.Class("IN.Tags.Apply").Extends("IN.Tags.Base").Constructor(function(q,p){$_STATISTICS.profile("Apply");
this.Parent(q,p);
this.callbackCallbacks=this.parseAttribute(p.callback,"callback");
this.resizeCallbacks=this.parseAttribute(p.onresize,"callback");
this.successCallbacks=this.parseAttribute(p.onsuccess,"callback");
this.closeCallbacks=this.parseAttribute(p.onclose,"callback");
this.clickCallbacks=this.parseAttribute(p.onclick,"callback");
this.errors={noSuccess:"no Email, Callback, or URL provided",noCompany:"no CompanyName or CompanyID provided",noJobTitle:"no JobTitle provided",noJobLocation:"no JobLocation provided",invalidEmail:"email is not a valid email address ({0})",invalidUrl:"url is not a valid URL ({0})"};
var r={companyname:{required:true,exception:["companyid"],error:this.errors.noCompany},companyid:{required:true,exception:["companyname"],error:this.errors.noCompany},jobtitle:{required:true,error:this.errors.noJobTitle},email:{required:true,exception:["url","callback"],error:this.errors.noSuccess,match:$_PATTERNS.email,invalidError:this.errors.invalidEmail},url:{required:true,exception:["email","callback"],error:this.errors.noSuccess,match:$_PATTERNS.url,invalidError:this.errors.invalidUrl},callback:{required:true,exception:["url","email"],error:this.errors.noSuccess},urlformat:{defaultValue:"json"},phone:{defaultValue:"optional"},coverletter:{defaultValue:"hidden"},size:{defaultValue:"large"},showtext:{defaultValue:"true"},questions:{transform:c},logo:{transform:o},meta:{defaultValue:"Apply with LinkedIn"}};
this.params=this.validateAttributes(p,r);
this.callbackOnly=(this.params.callback&&!(this.params.email||this.params.url))?true:false;
this.clickedOnce=false;
this.button=null;
this.applied=false;
this.textValues={normal:{buttonText:$_I18N("apply_with_linkedin"),helpText:$_I18N("get_hired_faster")},applied:{buttonText:$_I18N("youve_applied"),helpText:"You applied {0}."}};
this.jobData={companyName:this.params.companyname,companyId:this.params.companyid,jobLocation:this.params.joblocation,jobTitle:this.params.jobtitle,jobId:this.params.jobid};
this.jobData.jobHash=g(this.jobData);
this.globalId=this.jobData.jobId||this.jobData.jobHash;
f(this.globalId,this);
this.makeButton();
$_STATISTICS.profile("Apply",true)
}).Method("makeButton",function(){var t={jobId:this.params.jobid,companyId:this.params.companyid,companyName:this.params.companyname,jobTitle:this.params.jobtitle,jobLocation:this.params.joblocation,email:this.params.email,url:this.params.url,urlFormat:this.params.urlformat,logo:this.params.logo,phone:this.params.phone,coverLetter:this.params.coverletter,questions:this.params.questions,meta:this.params.meta,callback:this.params.callback,jobHash:this.jobData.jobHash,hasApplied:false};
for(var r in t){if(t.hasOwnProperty(r)&&(typeof t[r]===$_CONSTANTS.types.undef||(r==="urlFormat"&&!t.url))){delete t[r]
}}var v={themeColor:this.params.themecolor,api_key:IN.ENV.auth.api_key,isFormFill:this.callbackOnly};
var p=$_CONSTANTS.events,s=p.click;
function u(){var z,w;
if(this.winObj){this.callbackCallbacks.pop();
this.winObj.remove();
this.winObj=null
}t.hasApplied=this.getApplied();
v.data=JSON.stringify(t);
this.winObj=IN.UI.Apply().params(v);
var x=IN.Util.clone(this.jobData);
delete x.jobHash;
delete x.token;
var B={event:"undefined",job:x,meta:t.meta};
var y=this;
for(z=0,w=this.closeCallbacks.length;
z<w;
z++){var A=y.closeCallbacks[z];
this.winObj.onWindowRemove.subscribe(function(){B.event="close";
A(B)
},window)
}function C(){for(z=0,w=y.successCallbacks.length;
z<w;
z++){var D=y.successCallbacks[z];
y.winObj.onWindowRemove.subscribe(function(){B.event="success";
D(B)
},window)
}y.getApplyStatus()
}this.callbackCallbacks.push(C);
for(z=0,w=this.callbackCallbacks.length;
z<w;
z++){this.winObj.success(this.callbackCallbacks[z],window)
}for(z=0,w=this.resizeCallbacks.length;
z<w;
z++){this.winObj.onWindowResize.subscribe(this.resizeCallbacks[z],window)
}for(z=0,w=this.clickCallbacks.length;
z<w;
z++){B.event="click";
y.clickCallbacks[z](B)
}$_STATISTICS.recordAction("Apply",s);
this.winObj.place()
}this.button=new IN.Objects.Button({title:{normal:this.textValues.normal.buttonText,success:this.textValues.applied.buttonText},size:this.params.size,theme:"shernaz-fancy",showSuccessMark:true,normalizeSize:true});
this.button.place(this.el());
this.nodeId=this.button.mainNodeId;
if(this.params.showtext!=="false"){var q=document.createElement("span");
q.setAttribute("id",this.nodeId+"-text");
q.setAttribute("class","li-apply-text");
q.innerHTML=this.textValues.normal.helpText;
q.style.color="#666";
q.style.display="block";
q.style.fontSize="11px";
q.style.fontFamily="'Helvetica Neue', Arial, Helvetica, sans-serif";
q.style.textAlign="center";
q.style.marginTop="2px";
this.el().appendChild(q)
}this.getApplyStatus();
IN.Event.on(this.button,s,u,this);
IN.Event.on(IN,p.refresh,this.triggerRefresh,this);
IN.Event.on(IN,p.logout,this.triggerLogout,this)
}).Method("triggerRefresh",function n(){k(this.globalId,false);
this.getApplyStatus()
}).Method("triggerLogout",function h(){k(this.globalId,false)
}).Method("getApplied",function e(){return this.applied
}).Method("setApplied",function d(w,s){var r=this,q=0,y=0,v=0,t=0,p=0,x,z;
if(!this.button){window.setTimeout(function(){r.setApplied(w,s)
},200);
return
}this.applied=w;
if(w){this.button.setState($_CONSTANTS.states.success);
if(this.nodeId){x=this.textValues.applied;
if(this.params.showtext!=="false"){var u=s.fTime;
if(IN.ENV.js.lang===undefined||IN.ENV.js.lang==="en_US"){if(!u){u=" "+(IN.Util.relativeTime(s.timestamp)||"a few moments ago")
}u=IN.Util.formatString(x.helpText,u)
}z=IN.$Id(this.nodeId+"-text");
if(u){z.innerHTML=u
}}}}else{this.button.setState($_CONSTANTS.states.unSuccess);
if(this.nodeId){x=this.textValues.normal;
if(this.params.showtext!=="false"){z=IN.$Id(this.nodeId+"-text");
z.innerHTML=x.helpText
}}}}).Method("getApplyStatus",function l(){var p=m(this.globalId);
if(p.applied){this.setApplied(true,p.applied);
return
}if(p.sent){return
}var s=document.createElement("script"),q=document.getElementsByTagName("head")[0],r=IN.Util.createParams({job_hash:this.jobData.jobHash||"",job_id:this.jobData.jobId||"",token:IN.ENV.auth.anonymous_token||"",lang:IN.ENV.js.lang});
url=IN.ENV.widget.apply_counter_url+"?"+r;
s.type="text/javascript";
s.src=url;
i(this.globalId,"sent",true);
q.appendChild(s)
}).Static("handleStatus",function(p){a(p)
});
function a(q){var p=q.jobId||q.jobHash;
if(q.count===0){k(p,false)
}else{if(q.count>0){k(p,true,q)
}else{j[p]["applied"]=false;
j[p]["sent"]=false
}}}var o=function(r){if(!r){return undefined
}if($_PATTERNS.protocols.generic.test(r)){return r
}var q=IN.Util.getRootURL();
if($_PATTERNS.prefixes.forwardSlash.test(r)){return q+r
}var p=location.pathname.split("/").slice(0,-1).join("/");
return[q+p,r].join("/")
};
var c=function(r){if(!r){return undefined
}var q=$_CONSTANTS.types;
function u(y){var v=3,x=150,A={tooMany:"data-questions contains more than {0} questions - {1} question(s) truncated",invalid:"data-questions contains an invalid question: {1}",empty:"data-questions contains an empty question (question {0})",size:"data-questions contains a question that is too long (max {1} chars) (question {0})"};
if(IN.Util.isArray(y)){var z=y.length;
if(z>v){IN.Util.throwWarning(IN.Util.formatString(A.tooMany,v,(z-v)));
z=y.length=v
}for(var w=z;
w--;
){if((typeof y[w]!==q.object)||!y[w].question){IN.Util.throwWarning(IN.Util.formatString(A.invalid,y[w]));
throw new Error()
}y[w].question=IN.Util.trim(y[w].question);
if(y[w].question===""){IN.Util.throwWarning(IN.Util.formatString(A.empty,(w+1)));
throw new Error()
}if(y[w].question.length>x){IN.Util.throwWarning(IN.Util.formatString(A.size,(w+1),x));
throw new Error()
}}return y
}throw new Error()
}if(typeof r===q.string){var t=0;
do{var p=['{"values":',r,"}"].join("");
try{r=JSON.parse(p).values;
break
}catch(s){r=window[r];
if(typeof r!==q.string){break
}t++
}}while(t<2)
}return u(r)
}
...
if(this.width>0){a.width=this.width
}var d=new IN.Objects.SmartWindow({mode:"embedded",disableRefresh:true,url:IN.ENV.widget.lilaform_url}).params(a);
d.success(function(k){var m=[];
if(k.tracking&&k.tracking["onSubmit"]){this.submitUrl=k.tracking["onSubmit"]
}if(k.data){var g=document.getElementById(this.formid);
if(g){var f=document.getElementById(this.formid).elements;
for(var l=0,n=f.length;
l<n;
l++){var h=f[l].getAttribute("data-in-profile");
if(h){var o=k.data[h];
if(o){f[l].value=o;
m.push(h)
}}}}}var j=new Image();
if(k.tracking&&k.tracking["onClick"]){var e=k.tracking["onClick"].replace(/&amp;/g,"&");
e=e.replace(/__FIELDS_FILLED__/g,encodeURIComponent(m));
e=e.replace(/__API_KEY__/g,encodeURIComponent(IN.ENV.js.apiKey));
e=e.replace(/__UNIQUE_ID__/g,encodeURIComponent(this.uniqueid));
j.src=e
}},this);
d.place(this.el());
var c=false;
var b=this.formid;
IN.Event.on(this.formid,"submit",function(f){if(c){return
}var e=new Image();
if(this.submitUrl){f.preventDefault();
var g=this.submitUrl.replace(/&amp;/g,"&");
...
var i=this;
var h=window.setTimeout(function(){c=true;
document.getElementById(i.formid).submit();
e.onerror=e.onload=null
},1000);
e.onerror=e.onload=function(){c=true;
document.getElementById(this.formid).submit();
window.clearTimeout(h)
};
e.src=g+"&rand="+(new Date()).getTime()
}},this)
});
...
/* res://connect-min/dev/api/api.js */

(function(){var c=null,b=null,a=false;
Sslac.Static("IN.API").Static("get",function(d){if(!c){c=new IN.APIQueue();
function f(){window.setTimeout(function(){c.dispatch.call(c,true);
f()
},IN.ENV.api.queueInterval)
}f()
}var e=new IN.APIs[d]();
e.invoke=function(){c.invoke.apply(c,arguments)
};
return e
}).Static("Profile",function(){var d=IN.API.get("Profile");
d.ids.apply(d,arguments);
return d
}).Static("PeopleSearch",function(){if(arguments.length){throw new Error("PeopleSearch does not take any arguments.")
}var d=IN.API.get("PeopleSearch");
return d
}).Static("Connections",function(){var d=IN.API.get("Connections");
d.ids.apply(d,arguments);
return d
}).Static("NetworkUpdates",function(){var d=IN.API.get("NetworkUpdates");
d.ids.apply(d,arguments);
return d
}).Static("MemberUpdates",function(){var d=IN.API.get("MemberUpdates");
d.ids.apply(d,arguments);
return d
}).Static("Raw",function(){var d=IN.API.get("Raw");
d.url.apply(d,arguments);
return d
});
Sslac.Class("IN.APIQueue").Constructor(function(){this.callQueue=[];
this.maxQueue=10
}).Method("invoke",function(f,d,h,e,k){var g,p,m={},l,n=k.onSuccess||function(){},j=k.onFailure||function(){},o=k.scope||window;
function i(q){var r=(typeof(q)===$_CONSTANTS.types.string&&q!==""&&q!=="null");
var s={};
if(r){try{s=JSON.parse(q)
}catch(t){s.message="Error while parsing the server response to JSON (check 'raw_message' property for details)";
s.raw_message=q
}}return s
}g=function(q){var r=i(q);
if(typeof(r.raw_message)!==$_CONSTANTS.types.undef||typeof(r.errorCode)!==$_CONSTANTS.types.undef){j.call(o,r);
return
}n.call(o,r)
};
p=function(q){q=q.message;
var r=i(q);
j.call(o,r)
};
h.oauth_token=IN.ENV.auth.oauth_token||IN.ENV.auth.anonymous_token;
m={resource:IN.Util.trim(IN.ENV.url.api,"/")+"/"+IN.Util.trim(f,"/"),method:d,params:h,postBody:e,callback:g,failureCallback:p};
this.queue(m)
}).Method("queue",function(d){this.callQueue.push(d);
this.dispatch()
}).Method("dispatch",function(j){var e=this,h=null;
if(!b){h=["mode=cors"].join("&");
b=new IN.Lib.easyXDM.Rpc({remote:IN.ENV.url.api_xd_html+"#"+h,hash:true,swf:IN.ENV.images.secure_xdswf,onReady:function k(){a=true;
e.dispatch(j)
}},{local:{},remote:{ajax:{}}});
return
}if(!a){return
}if(!j&&this.callQueue.length<this.maxQueue){return
}if(this.callQueue.length===0){return
}for(var f=0,d=this.callQueue.length;
f<d;
f++){var g=this.callQueue[f];
b.ajax({method:g.method,url:g.resource,params:g.params,postBody:g.postBody},g.callback,g.failureCallback)
}this.callQueue=[]
})
})();

/* res://connect-min/dev/apis/base.js */

Sslac.Class("IN.APIs.Base").Constructor(function(){this.isLinked=false;
this.useParams={};
this.useMethod="GET";
this.postBody=null;
this.executed=false;
...
}).Method("params",function(a){if(this.executed){throw new Error("Command has executed. You can't append additional properties.")
}if(!(a instanceof Object)){throw new Error("params() must receive a key-value-pair like object.")
}for(name in a){this.useParams[name]=a[name]
}return this
}).Method("setParam",function(c,b,e){if(this.executed){throw new Error("Command has executed. You can't append additional properties.")
}var g=[].slice.apply(b);
if(IN.Util.isArray(g[0])){g=g[0]
}else{if(!IN.Util.isArray(g)){g=g.split(",")
}}for(var d=0,a=g.length;
d<a;
d++){var f=g[d];
g[d]=this.transformField(f,e)
}if(g.length){this.useParams[c]=g
}return this
}).Method("method",function(a){if(this.executed){throw new Error("Command has executed. You can't append additional properties.")
}this.useMethod=a.toUpperCase();
return this
}).Method("body",function(a){if(this.executed){throw new Error("Command has executed. You can't append additional properties.")
}this.postBody=a;
return this
}).Method("result",function(){$_STATISTICS.instance("apis:"+this.type);
this.addHandler(this.handlers.raw,[].slice.apply(arguments));
this.get();
return this
}).Method("error",function(){this.addHandler(this.handlers.errors,[].slice.apply(arguments));
this.get();
return this
}).Method("name",function(){throw new Error("name() must be defined")
}).Method("resource",function(){throw new Error("resource() must be defined")
}).Method("getMethod",function(){return this.useMethod
}).Method("format",function(f,g){g=g||"values";
var a=false,b=g.split(".")[0],i={},d;
try{d=IN.Util.findIn(f,g);
a=true
}catch(h){d=f
}if(a){for(var c in f){if(c==b){continue
}i[c]=f[c]
}}return{values:d,meta:i}
}).Method("getParams",function(){return this.useParams
}).Method("getPostBody",function(){return this.postBody||""
}).Method("getResource",function(g){var f=this.resource(g);
var b=this.getParams();
for(var a in b){var e=a.toUpperCase();
var d=new RegExp("\\{"+e+"\\}","g");
var c=(IN.Util.isArray(b[a]))?b[a].join(","):b[a];
f=f.replace(d,c)
}f=f.replace(/\{.*?\}/g,"");
...
f=IN.Util.trim(f,":");
return f
}).Method("getParameters",function(h){var g=this.resource(h),d=this.getParams(),b={},c=0,a;
for(a in d){var f=a.toUpperCase();
var e=new RegExp("\\{"+f+"\\}","g");
if(!g.match(e)){b[a]=d[a];
c++
}}if(h){for(a in h){b[a]=h[a];
c++
}}return(c)?b:false
}).Method("addHandler",function(d,a){var c=window,b=function(){};
if(a[1]){c=a[1]
}if(a[0]){b=a[0]
}d.push({fn:b,scope:c,ran:false});
if(this.storedSuccessResults){this.handleSuccessResults(this.storedSuccessResults)
}if(this.storedFailureResults){this.handleFailureResults(this.storedFailureResults)
}}).Method("runHandler",function(d,f,c){for(var b=0,a=d.length;
b<a;
b++){var e=d[b];
if(!e.ran){e.fn.call(e.scope,f,c)
}}}).Method("markHandlerAsRan",function(c){for(var b=0,a=c.length;
b<a;
b++){c[b].ran=true
}}).Method("handleSuccessResults",function(a){this.storedSuccessResults=a;
this.runHandler(this.handlers.raw,a);
this.markHandlerAsRan(this.handlers.raw);
return this
}).Method("handleFailureResults",function(a){this.storedFailureResults=a;
this.runHandler(this.handlers.errors,a)
}).Method("get",function(){if(this.executed||this.isLinked){return this
}this.executed=true;
var a=this.getParameters()||{},b=this.getResource(),d=this.getMethod(),c=this.getPostBody();
this.invoke(b,d,a,c,{onSuccess:this.handleSuccessResults,onFailure:this.handleFailureResults,scope:this});
return this
}).Method("transformField",function(f,e){var c=null;
if(typeof(f)=="string"){c=f;
if(c=="me"){c="~"
}if($_PATTERNS.prefixes.urlEq.test(c)){c="url="+encodeURIComponent(c.replace($_PATTERNS.prefixes.urlEq,""))
}if($_PATTERNS.protocols.generic.test(c)){c="url="+encodeURIComponent(c)
}if(!e){c=c.replace($_PATTERNS.context.upperCase,"-$1").toLowerCase()
}}else{if(f instanceof IN.APIs.Base){c=f.getResource(true)
}else{if(IN.Util.isArray(f)){c=[];
for(var d=0,a=f.length;
d<a;
d++){c.push(this.transformField(f[d]))
}c=c.join(",")
}else{if(typeof(f)=="object"){c=[];
for(var b in f){c.push(b+":("+this.transformField(f[b])+")")
}c=c.join(",")
}else{throw new Error("Unknown Type: "+f)
}}}}return c
});

/* res://connect-min/dev/apis/raw.js */

Sslac.Class("IN.APIs.Raw").Extends("IN.APIs.Base").Constructor(function(){this.Parent(arguments);
this.useMethod="GET";
this.type="Raw"
}).Method("url",function(){return this.setParam("raw",[].slice.apply(arguments),true)
}).Method("name",function(){return"raw"
}).Method("resource",function(a){if(!this.getParams().raw||this.getParams().raw.length===0){throw new Error("url must be set either through the constructor or through .url()")
}if(a){return"{RAW}"
}else{return"/{RAW}"
}});

/* res://connect-min/dev/apis/profile.js */

Sslac.Class("IN.APIs.Profile").Extends("IN.APIs.Base").Constructor(function(){this.Parent(arguments);
this.defaultFields=["id","first-name","last-name","headline","picture-url"];
this.setParam("fields",this.defaultFields);
this.type="Profile"
}).Method("ids",function(){return this.setParam("ids",[].slice.apply(arguments),true)
}).Method("fields",function(){return this.setParam("fields",[].slice.apply(arguments))
}).Method("name",function(){return"people.get"
}).Method("resource",function(){var c=this.getParams().ids;
if(!c||c.length===0){throw new Error("ids must be set either through the constructor or through .ids()")
}if(!IN.ENV.auth.oauth_token){for(var b=0,a=c.length;
b<a;
b++){if(c[b]==="~"){throw new Error("You need to authenticate in order to use 'me' or '~' as a valid profile identifier")
}}}return"/people::({IDS}){ISPUBLIC}:({FIELDS})"
});

/* res://connect-min/dev/apis/peoplesearch.js */

Sslac.Class("IN.APIs.PeopleSearch").Extends("IN.APIs.Base").Constructor(function(){this.Parent(arguments);
this.type="PeopleSearch"
}).Method("facets",function(){var a=[].slice.apply(arguments);
if(!a||!a.length){a=["name","code",{buckets:["code","name","count"]}]
}return this.setParam("facets",a)
}).Method("fields",function(){return this.setParam("fields",[].slice.apply(arguments))
}).Method("name",function(){return"peoplesearch.get"
}).Method("resource",function(){var a=[];
a.push("num-results");
if(this.getParams().fields){a.push("people:({FIELDS})")
}else{a.push(["people:(",["id","first-name","last-name","headline","picture-url"].join(","),")"].join(""))
}if(this.getParams().facets){a.push("facets:({FACETS})")
}return"/people-search:("+a.join(",")+")"
}).Method("format",function(a){a=this.Parent(a,"people.values");
return a
});

/* res://connect-min/dev/apis/connections.js */

Sslac.Class("IN.APIs.Connections").Extends("IN.APIs.Base").Constructor(function(){this.Parent(arguments);
this.type="Connections"
}).Method("ids",function(){return this.setParam("ids",[].slice.apply(arguments),true)
}).Method("fields",function(){return this.setParam("fields",[].slice.apply(arguments))
}).Method("name",function(){return"connections.get"
}).Method("resource",function(a){if(!this.getParams().ids||this.getParams().ids.length===0){throw new Error("ids must be set either through the constructor or through .ids()")
...
/* res://connect-min/dev/apis/networkupdates.js */

Sslac.Class("IN.APIs.NetworkUpdates").Extends("IN.APIs.Base").Constructor(function(){this.Parent(arguments);
this.setParam("ids",["me"],true);
this.type="NetworkUpdates"
}).Method("ids",function(){var a=[].slice.apply(arguments);
if(!a.length){return this
}if(a[0]!=="me"||a.length>1){throw new Error("ids are restricted to the current user")
}return this.setParam("ids",["me"],true)
}).Method("fields",function(){return this.setParam("fields",[].slice.apply(arguments))
}).Method("name",function(){return"networkupdates.get"
}).Method("resource",function(a){return"/people/{IDS}/network/updates:({FIELDS})"
});

/* res://connect-min/dev/apis/memberupdates.js */

Sslac.Class("IN.APIs.MemberUpdates").Extends("IN.APIs.Base").Constructor(function(){this.Parent(arguments);
this.params({scope:"self"});
this.type="MemberUpdates"
}).Method("ids",function(){return this.setParam("ids",[].slice.apply(arguments),true)
}).Method("fields",function(){return this.setParam("fields",[].slice.apply(arguments))
}).Method("name",function(){return"memberupdates.get"
}).Method("resource",function(a){if(!this.getParams().ids||this.getParams().ids.length===0){throw new Error("ids must be set either through the constructor or through .ids()")
...
/* res://connect-min/dev/wiz.js */

"Have you tried debug=1 lately?";
